...
- **Charge unknown outcome ≠ failure**: order/plan timeouts and network errors stay `pendente`, and unresolved orders must not auto-expire; a delayed signed callback may still be the authoritative success.
- **Automatic payout reconciliation must be fair**: each pending/errored status check advances its retry timestamp so a fixed-size oldest-first batch cannot starve newer payouts. Duplicate status GETs are safe; money transitions remain conditional and transactional.
- **Webhook failure alerts are durable before Push**: persist with a DB-unique idempotency key, then attempt Web Push. Provider retries and multiple replicas must not create duplicate in-app alerts.
...
- **Money flow (critical):** buyer pays → AppyPay platform account (NOT merchant's e-kwanza account directly); Linkealls wallet_ledger mirrors the balance; owner gets money by requesting a "saque" (payout) via Operations API (SendToCustomer/SendKWiKToCustomer) after accumulating ≥1,000 Kz. EKWANZA_MERCHANT_IDENTIFIER is for transaction mapping/reconciliation, not direct credit to that account.
