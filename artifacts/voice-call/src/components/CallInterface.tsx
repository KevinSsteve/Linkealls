  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        ...
        {active && (
          <span
            className={`absolute inset-0 rounded-full ${color.replace("border-", "border-")} opacity-40 animate-ping`}
...
export function CallInterface() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-950 text-white gap-10 p-6">
      {/* Title */}
      ...
      <div className="flex items-center gap-2.5">
        <span
          className={`w-2.5 h-2.5 rounded-full ${STATUS_COLOR[callState]}`}
        />
        <span className="text-base text-gray-300">{STATUS_LABEL[callState]}</span>
      </div>

      {/* Speaking indicators — only shown during active call */}
      {callState === "active" && (
